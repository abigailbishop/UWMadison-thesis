\chapter{The Askaryan Radio Array's Five Station Analysis \label{chap:5sa_analysis}}
  ...
  Any events that survive all analysis cuts (cleaning and LDA) are then passed through an isolation condition that removes any events that occurred between 5 seconds and 5.8 hours of each other, as we do not expect to find multiple neutrino events in close succession, except events that observe multiple particles or cascades (muon and tau tracks of stochastic losses and a tau decay), like those quantified in Fig.~\ref{fig:secondaries_contribution}. 
  ...
  Lastly, in Section~\ref{sec:5sa_results}, we use the expected background rates and the number of passing events to establish an upper limit on the flux of UHE neutrinos, which I took the lead in calculating. 
...
\section{The Linear Discriminant Analysis \label{sec:lda}}
  ...
  This LDA is trained on a signal dataset composed of simulated neutrino events following the global experimental upper limit on the cosmic neutrino set (defined by \citep{icecube_search-ehe} and \citep{anita_iv}), and a background dataset composed of all events from each station's cleaned 10\% sample.   
...
\section{Optimization \label{sec:optimization}}
  ...
  After the thermal background LDA model determined in Section~\ref{sec:lda} and the surface event elevation models determined in Section~\ref{sec:zc_slc} are set, we feed them into an algorithm that determines the best LDA cut for each array-wide configuration and the best Zenith Cut for each station. 
  ...
  This allows the optimizer to allocate the estimated background amongst all available stations which means that detectors with greater exposure can have less strict LDA cuts, enabling both more backgrounds and, ideally, neutrinos to pass all analysis cuts.
  ...
  The optimization is designed around the $5\sigma$ discovery potential of a flux 
  \begin{equation} 
    \phi(E) = \phi_0^{\text{disc}}f(E)
    \label{eq:optimizer_flux}
  \end{equation}  
  where $\phi_0^{\text{disc}}$ is the flux normalization at an energy of $E_0 = 10^{18.5}$~eV (the central energy of the logarithmic energy range used in this analysis) and $f(E)$ parameterizes the spectral shape. 
  The shape function is defined such that $f(E)$ is unit-less, $f(E_0)=1$, and $f(E)$ follows the shape of the global experimental upper limit defined by \citep{icecube_search-ehe} and \citep{anita_iv}. 
  The optimization script specifically minimizes the flux normalization as
  \begin{equation}
    \phi^{\text{disc}}_0 (\Sigma, \Theta) = \frac{\text{FC}(n_{\text{disc}}(B), B)}{\int f(E) \mathcal{E}(E) dE }
    \label{eq:optimization}
  \end{equation}
  where $\Sigma$ is the collection of thermal LDA cuts and $\Theta$ is the collection of surface cuts for each station which are being tuned by the optimizer.
  $\text{FC}(n_{\text{disc}}(B), B)$ is the Feldman Cousins 90\% confidence level upper limit given in \citep{feldman_cousins}, $n_{\text{disc}}$ is the number of events required for a $5\sigma$ discovery, and $B$ is the expected background (described in the next paragraph). 
  $n_{\text{disc}}$ is determined via the Poisson cumulative distribution function
  \begin{equation}
    C(n_{\text{disc}}; B) = \sum_{k=0}^n \frac{B^k e^{-B}}{k!} \geq 1 - \beta
    \label{eq:ndisc_B}
  \end{equation}
  where $\beta$ is the one-sided tail probability corresponding to the targeted $5\sigma$ significance.
  To finish defining Equation~\ref{eq:optimization},  $\mathcal{E}(E)$ is the analysis-level exposure (the term defined in Equation~\ref{eq:exposure_trigger} multiplied by the analysis efficiency).
  
  The expected background, $B$, is defined as the sum of expected backgrounds from the Geometry Cut (defined in Section~\ref{sec:gc}), the Zenith Cut (described in Section~\ref{sec:zc_slc}), and the thermal LDA cut (described in Section~\ref{sec:lda}) after accounting for overlaps in some background populations.
  The background rate from the Geometry Cut is constant over the full array (geometry cuts are applied to a station's full livetime, they do not change over time) and is designed for a background event rate of $2 \times 10^{-8}$ events per region being cut, resulting in a total array-wide background, $b_{\text{geom}}$, of $4.2 \times 10^{-7}$ events.
  The Geometry Cut's background estimate is also assumed to have negligible influence on the Zenith Cut and thermal LDA cut and is therefore treated as independent of these cuts.
  The background rate for the Zenith Cut and thermal LDA cut are correlated, however. 
  The Zenith Cut's background is defined as
  \begin{equation}
    b_{\text{surf}}(\Sigma, \Theta) = \sum_s b_s(\theta_{\text{th}, s}) ~ f_s(\Sigma)
    \label{eq:surface_background}
  \end{equation}
  and the thermal LDA cut is defined as
  \begin{equation}
    b_{\text{therm}}(\Sigma, \Theta) = \sum_{a,s,c} w_{a, s, c} ~ b_{s,c}(\sigma_{\text{th}, a, s}) ~ f_s(\theta_{\text{th},s })
    \label{eq:thermal_background}
  \end{equation}
  where $a$ corresponds to an array-wide configuration, $s$ to a station, and $c$ to a station configuration.
  For Equation~\ref{eq:thermal_background}, $b_s(\theta_{\text{th}, s})$ is the surface background for station $s$ over the full analysis livetime, calculated as the integral of events existing in the surface model beyond the cut on reconstructed elevation angle $\theta_{\text{th}}$.
  $f_s(\Sigma)$ is a correction factor accounting for the fraction of leaked surface events on station $s$ which survive the thermal LDA cut. 
  For Equation~\ref{eq:thermal_background}, $b_{s,c}(\sigma_{\text{th}, a, s})$ is the thermal background rate of station $s$ in configuration $c$ for the thermal LDA cut $\sigma_{\text{th}, a, s}$ for station $s$ in the context of array-wide configuration $a$.
  Since the thermal LDA cut changes as a function of time, a weighting factor, $w_{a, s, c}$, is applied to each term in the sum and is defined as the livetime of station $s$ in array-wide configuration $a$ divided by the full livetime of the corresponding station-configuration $c$.
  Similarly to the Zenith Cut's background calculation, $f_s(\theta_{\text{th},s })$ is a correction factor applied to the thermal LDA background rate which accounts for the fraction of events removed by the Zenith Cut on station $s$.
  In total, the array-wide background rate is the sum of these three contributions:
  \begin{equation}
    B(\Sigma, \Theta) = b_{\text{geom}} + b_{\text{surf}}(\Sigma, \Theta) + b_{\text{therm}}(\Sigma, \Theta)
    \label{eq:total_background}
  \end{equation}
  ...
  The total array-wide background event rate is 0.13 events, of which $4.2 \times 10^{-7}$ events are from the Geometry Cut, 0.045 events are from the surface cut, and 0.086 events are from the thermal LDA cut.
  The expected number of observed events, $n_{\text{disc}}$, is 0. 
...
\section{Post-Unblinding Analysis \label{sec:unblinding}}
  ...
  There is also one final cut to remove any array-wide temporal clusters of events, called the Time Isolation Cut.
  ...
  \subsection{Surface Cut Cross Checks}
    ...
    The A2 model built on the 10\% data passed our check as the p-value of the observed log-likelihood compared to the pseudoexperiments was greater than 0.05.
  ...
  \subsection{Thermal Background Cross Checks}
    ...
    \begin{figure}
      ...
      \caption{
        ...
        These events are fit to an exponential (green line) plotted along the model built on the 10\% data multiplied by 10 (red dotted line).
    ...
    \begin{figure}
      ...
      \caption{
        Distribution of the log-likelihood of pseudoexperiments built on 100\% A2 Configuration 4 data (blue distribution) compared to the log-likelihood of the observed 100\% data (red line) both calculated against the 10\% background model scaled up by a factor of 10.
      }
...
\section{Inspection of Passing Events \label{sec:passing_events}}
    ...
    \subsection{A Posterior Investigation of the Passing Event}
      ...
      The analysis presented in \citep{ara_a23} also discovered 7 events of this nature which gave rise to the original Spark Event Cut.% \footnote{\url{https://aradocs.wipac.wisc.edu/cgi-bin/DocDB/ShowDocument?docid=2032}}.
...
\section{Results\label{sec:5sa_results}}
    ...
    We multiply this by our expected untagged calibration pulser event rate of $10^{-4}$ for each station to achieve a background of $2\times 10^{-8}$ events per window.  
